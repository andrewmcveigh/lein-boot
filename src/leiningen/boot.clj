...
(defn boot-server [port default-mappings handlers]
  `(do
     ;~namespace-decl
     (ns ~'boot)
     ...
     (defn ~'start-server []
       (let [path# ~(find-webapp-root)
         ...
         (~'add-servlet-mappings (~'->default-servlet-mapping ~default-mappings))
...
(defn boot [project & args]
  (let [{:keys [port]}
        ...
        _ (clojure.pprint/pprint (boot-server port mappings handlers))
        ...
        project (update-in project
                           [:repl-options :init]
                           #(list 'do % (boot-server port mappings handlers)))
